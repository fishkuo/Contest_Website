...
app.post("/add_child_review", (req, res) => {
...
app.post("/add_elderly_review", (req, res) => {
    var date_ob = new Date();
    var id = req.body.name;
    var city = req.body.city;
    var district = req.body.district;
    ...
    const QUERY_STRING1 = `INSERT INTO \`elderlyMessage\` (name, date, rating) VALUES ("${id}", "${date}", ${rating})`;
    pool.getConnection(function (err, connection) {
        connection.query(QUERY_STRING1, function (error, result, fields) {
            if (error) {
                console.log("Query Failed: ", error);
    ...
    const QUERY_STRING2 = `UPDATE elderlyCareSystem SET rating_amount = rating_amount + 1, \
                          rating_score = rating_score + ${rating}, rating_average = rating_score / rating_amount WHERE name = "${id}"`;
    pool.getConnection(function (err, connection) {
        connection.query(QUERY_STRING2, function (error, result, fields) {
            if (error) {
...
app.get("/elderly_district", (req, res) => {
    const QUERY_STRING = `SELECT name, type, address, rating_average, google_rating \
